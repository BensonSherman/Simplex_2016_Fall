...
class Application
{
	...
	std::vector<std::vector<vector3>> pointGrid;//used for indexing stops
	std::vector<uint> fRoutes; //storing the route
	std::vector<float> fTimers; //timer for each orbit
	std::vector<uint> uClocks;//need multiple clocks for multiple timers
